...
* timeouts
	...
	** timeout parameters
		*** `connectTimeout`
			**** prevents blocking the caller
			**** DNS lookup timing is part of it
		*** `readTimeout`
			**** [time of connection establishment, time of connection establishment + `readTimeout`]
			**** returning the response -- takes -- too long

* if server is NOT running or available -> packet results in _connection refused_
	** -> communication ends either
		*** error message or
		*** fallback
	** can happen _BEFORE_ the `connectTimeout`
